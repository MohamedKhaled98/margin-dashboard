...
export async function getProjectsListData() {
    ...
    const entries = await TimesheetEntry.find().lean();
    const salaries = await MonthlySalary.find().lean();
  
    const data = [];
  
    for (const project of projects) {
      const projectEntries = entries.filter(
        entry => entry.refCode === project.refCode
      );
      ...
      for (const entry of projectEntries) {
        ...
        const employeeMonthlyHours = entries
          .filter(
            e =>
              e.employeeNo === entry.employeeNo &&
              e.month === entry.month &&
              e.year === entry.year
          )
          .reduce((sum, e) => sum + e.hours, 0);
  
        if (!employeeMonthlyHours) continue;
  
        const hourlyRate =
          empSalary.amount / employeeMonthlyHours;
  
        totalCost += hourlyRate * entry.hours;
      }
  
      const profit = project.price - totalCost;
      const margin = project.price
        ? profit / project.price
        : 0;
